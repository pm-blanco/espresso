...
class ReactionMethods(ut.TestCase):
    ...
    def check_interface(self, method, kT, exclusion_range, gamma):
    ...
    def test_interface(self):
        ...
        self.check_interface(method, kT=1.5, exclusion_range=0.8, gamma=1.2)

        # constant pH ensemble
        method = espressomd.reaction_ensemble.ConstantpHEnsemble(
            kT=1.5, exclusion_range=0.8, seed=12, constant_pH=10)
        self.check_interface(method, kT=1.5, exclusion_range=0.8, gamma=1.2)

        # Widom insertion
        method = espressomd.reaction_ensemble.WidomInsertion(kT=1.6, seed=12)
        self.check_interface(method, kT=1.6, exclusion_range=0., gamma=1.)
