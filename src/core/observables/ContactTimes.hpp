...
/**  Tracks the time evolution of contacts between `ids` and `target_ids` within
 * a given `contact_threshold`
 */
...
public:
  ...
  explicit ContactTimes(std::vector<int> const &ids,
                        std::vector<int> const &target_ids,
                        double contact_threshold)
      : PidTimeObservable(ids, target_ids, contact_threshold) {
    if (this->ids().size() < 1) {
      throw std::runtime_error("At least 1 particle in ids is required");
    }
    if (this->target_ids.size() < 1) {
      throw std::runtime_error("At least 1 particle in target_ids is required");
    }
    if (contact_threshold < 0) {
      throw std::runtime_error(
          "The contact threshold must be a positive number.");
    }
    ...
    this->first_contact_times = std::vector<std::vector<double>>(
        n_ids, std::vector<double>(n_target_ids, time));
    this->contacts = std::vector<std::vector<bool>>(
        n_ids, std::vector<bool>(n_target_ids, false));
  }
   ...
   */
  std::vector<double> get_contact_times_series() const {
  ...
  std::vector<double> get_instantaneous_contact_times() const {
    ...
    for (auto &id1 : ids1) {
      for (auto &id2 : ids2) {
        ...
        if (this->contacts[index1]
                          [index2]) { // index1 and index2 are now in contact
    ...
    for (auto &id1 : ids1) {
      for (auto &id2 : ids2) {
        ...
        if (dist < contact_threshold) { // pid1 and pid2 are in contact now
          if (!(this->contacts[index1][index2])) { // but they were not in
                                                   // contact before!
          ...
          if (this->contacts[index1]
                            [index2]) { // index1 and index2 are not in contact
                                        // now but they were before
  ...
  std::vector<std::size_t> shape() const override { return {}; }
};
